...
export const deleteCareer = (id) =>
  api.delete(`/careers/${id}`).then((r) => r.data);

// Applications
export const listAllApplications = (params = {}) =>
  api.get("/careers/applications", { params }).then((r) => r.data);
export const listApplicationsForJob = (id, params = {}) =>
  api.get(`/careers/${id}/applications`, { params }).then((r) => r.data);
export const updateApplicationStatus = (id, payload) =>
  api.patch(`/careers/applications/${id}/status`, payload).then((r) => r.data);

// Homepage slides
export const listSlides = () => api.get("/homepage/slides").then((r) => r.data);
export const createSlide = (data) =>
  api.post("/homepage/slides", data).then((r) => r.data);
export const updateSlide = (id, data) =>
  api.patch(`/homepage/slides/${id}`, data).then((r) => r.data);
export const deleteSlide = (id) =>
  api.delete(`/homepage/slides/${id}`).then((r) => r.data);
