...
import {
  ...
  Globe, Archive, Eye, EyeOff, ExternalLink,
} from "lucide-react";
...
function NewPortfolioModal({ open, busy, onClose, onSubmit }) {
  const [form, setForm] = useState({ title: "", slug: "", excerpt: "", category: "", status: "draft" });
  const set = (k) => (e) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-[28px] bg-white p-6 shadow-2xl">
        <div className="mb-5 text-base font-black text-slate-900">New portfolio item</div>
        <div className="grid gap-4">
          <div className="grid gap-2">
            <label className="text-[11px] font-black tracking-[0.18em] text-slate-500">TITLE *</label>
            <input value={form.title} onChange={set("title")} placeholder="Brand Campaign for Acme Co." className="h-11 rounded-2xl border border-slate-200 bg-white px-4 text-sm outline-none focus:ring-4 focus:ring-black/5" />
          </div>
          <div className="grid gap-2">
            <label className="text-[11px] font-black tracking-[0.18em] text-slate-500">SLUG *</label>
            <input
              value={form.slug}
              onChange={(e) => setForm((f) => ({ ...f, slug: slugify(e.target.value), _slugEdited: true }))}
          ...
          <div className="grid gap-2">
            ...
            <textarea value={form.excerpt} onChange={set("excerpt")} rows={2} placeholder="Short description shown in listings..." className="resize-none rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:ring-4 focus:ring-black/5" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="grid gap-2">
              <label className="text-[11px] font-black tracking-[0.18em] text-slate-500">CATEGORY</label>
              <input value={form.category} onChange={set("category")} placeholder="Branding, Social, Video..." className="h-11 rounded-2xl border border-slate-200 bg-white px-4 text-sm outline-none focus:ring-4 focus:ring-black/5" />
            </div>
            <div className="grid gap-2">
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <Button onClick={() => onSubmit(form)} loading={busy} disabled={!form.title.trim() || !form.slug.trim()}>
            <Plus size={14} /> Create
          </Button>
        </div>
      </div>
...
export default function PortfolioPage() {
  ...
  const filtered = useMemo(() => {
    return items.filter((x) => {
      const matchQ = !q || [x.title, x.excerpt, x.category, ...(x.tags || [])].join(" ").toLowerCase().includes(q.toLowerCase());
  ...
  return (
    <div className="grid gap-6">
      <PageHeader
        ...
        breadcrumb={[{ label: "Portal", to: "/portal" }, { label: "Portfolio" }]}
        right={
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        <StatCard icon={Layers} label="Total"     value={stats.total}     color="indigo"  />
        <StatCard icon={Eye}    label="Published" value={stats.published} color="emerald" />
        <StatCard icon={EyeOff} label="Draft"     value={stats.draft}     color="slate"   />
        <StatCard icon={Archive} label="Archived" value={stats.archived}  color="rose"    />
      </div>
      ...
      <FilterBar
        ...
        filters={[
          { label: "status", value: statusFilter, onChange: (v) => setStatusFilter(v), options: [
      ...
      {loading ? (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {[1, 2, 3].map((i) => <Skeleton key={i} className="h-48 w-full rounded-[28px]" />)}
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {filtered.map((item) => (
            <div
              ...
              onKeyDown={(e) => { if (e.key === "Enter") nav(`/portal/portfolio/${item._id}`); }}
            ...
            >
              ...
              {item.coverMedia?.url ? (
                <div className="h-40 w-full overflow-hidden rounded-2xl bg-slate-100">
                  <img src={item.coverMedia.url} alt={item.title} className="h-full w-full object-cover" />
                </div>
              ) : (
              ...
              <div>
                <div className="text-base font-black text-slate-900 group-hover:text-slate-700">{item.title}</div>
                {item.excerpt && (
                  <div className="mt-1 text-sm text-slate-500 line-clamp-2">{item.excerpt}</div>
                )}
              </div>
              ...
              <div className="mt-auto flex items-center justify-between gap-2 pt-2 border-t border-slate-100">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <ExternalLink size={14} className="text-slate-400 group-hover:text-slate-700 transition" />
                </div>
              </div>
            </div>
          ))}
