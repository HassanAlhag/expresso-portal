...
export default function LeadActivitiesTab({ lead }) {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const res = await listActivities({ entityType: "lead", entityId: lead._id, limit: 100 });
  ...
  useEffect(() => { load(); }, [lead?._id]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleCreated = (newItem) => setItems((prev) => [newItem, ...prev]);
  const handleDeleted = (id) => setItems((prev) => prev.filter((x) => x._id !== id));
